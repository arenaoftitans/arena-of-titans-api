...
from aot.api.utils import AotError, AotErrorToDisplay, AotFatalError, RequestTypes
...
async def test_reconnect_creating_game(api, game):  # noqa: F811
    ...
    api._cache = MagicMock()
    api._cache.has_game_started = AsyncMock(return_value=False)
    api._cache.get_player_index = AsyncMock(return_value=0)
    api._reconnect_to_game = MagicMock()
    api._get_initialized_game_message = AsyncMock(return_value={"message": "smth"})
...
async def test_reconnect_creating_game_slot_freed(api, game):  # noqa: F811
    ...
    api._cache = MagicMock()
    api._cache.has_game_started = AsyncMock(return_value=False)
    api._cache.get_player_index = AsyncMock(side_effect=IndexError)
    api._reconnect_to_game = MagicMock()
    api._get_initialized_game_message = AsyncMock(return_value={"message": "smth"})
    api._game_id = "game-id"
    api._message = {
...
async def test_reconnect_reconnect_to_game(api, game):  # noqa: F811
    ...
    api._cache = MagicMock()
    api._cache.has_game_started = AsyncMock(return_value=True)
    api._get_game = AsyncMock(return_value=game)
    api._save_game = AsyncMock()
    api._reconnect_to_game = MagicMock()
    api._message = {
        "player_id": "player_id",
